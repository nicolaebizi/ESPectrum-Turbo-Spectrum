...
uint8_t MemESP::turboFE_REG = 0;
uint8_t MemESP::turboFE_CFG = 0;
...
void MemESP::TurboSpectrumApplyMemoryMap() {

    if (Config::romSet48 != "48Kcs")
        return;

    switch (MemESP::turboFE_CFG & 0x60) {

        case 0x00:
            // 0000-3FFF = ROM
            // 4000-7FFF = RAM0
            // 8000-BFFF = RAM1
            // C000-FFFF = RAM5 (VIDEO)
            MemESP::ramCurrent[0] = MemESP::rom[0];
            MemESP::ramCurrent[1] = MemESP::ram[0];
            MemESP::ramCurrent[2] = MemESP::ram[1];
            MemESP::ramCurrent[3] = MemESP::ram[5];
            break;
        ...
        case 0x20:
            ...
            // 8000-BFFF = RAM1
            // C000-FFFF = RAM5 (VIDEO)
            MemESP::ramCurrent[0] = MemESP::ram[6];
            MemESP::ramCurrent[1] = MemESP::ram[0];
            MemESP::ramCurrent[2] = MemESP::ram[1];
            MemESP::ramCurrent[3] = MemESP::ram[5];
            break;
        ...
        case 0x40:
            // VHDL: when others -> ram_page = 000
            // ROM is disabled because FE_CFG[6] = 1.
            MemESP::ramCurrent[0] = MemESP::ram[0];
            MemESP::ramCurrent[1] = MemESP::ram[0];
            MemESP::ramCurrent[2] = MemESP::ram[0];
            MemESP::ramCurrent[3] = MemESP::ram[0];
            break;

        case 0x60:
            // 0000-3FFF = RAM1
            // 4000-7FFF = RAM5 (VIDEO)
            // 8000-BFFF = RAM6
            // C000-FFFF = RAM0
    ...
    printf("[TURBO MAP] FE_REG=%02X FE_CFG=%02X  MAP=%p %p %p %p  RAM6=%p\\n",
           MemESP::turboFE_REG,
           MemESP::turboFE_CFG,
           MemESP::ramCurrent[0],
           MemESP::ramCurrent[1],
...
void MemESP::Reset() {
    ...
    if (Config::romSet48 == "48Kcs") {
        MemESP::turboFE_REG = 0;
        MemESP::turboFE_CFG = 0;

        printf("[TURBO RESET] romSet48='%s' arch='%s'\\n",
