...
## Turbo Spectrum / CoBra-2

This repository is an ESPectrum-based project extended with support for
the **Turbo Spectrum (CoBra-2)** architecture.

The original ESPectrum functionality and attribution are retained. The
Turbo Spectrum work adds hardware-specific memory mapping, ROM/RAM
switching and related emulation support.

### Turbo Spectrum support currently includes

- Turbo Spectrum custom ROM support.
- Turbo Spectrum FE register and configuration handling.
- Turbo Spectrum ROM/RAM switching.
- Turbo Spectrum RAM6 mapping.
- Turbo Spectrum 64K DRAM memory configuration.
- Turbo Spectrum-specific CPU memory write handling.
- Turbo Spectrum video-memory handling.
- Turbo Spectrum startup/logo animation support.
- Turbo Spectrum boot-sequence support.
- Diagnostic logging for Turbo Spectrum memory-map transitions.

The implementation follows the published Turbo Spectrum technical
documentation.

### Turbo Spectrum memory configuration
...
A distinction is maintained between the programmed FE register and the
active FE configuration, matching the documented Turbo Spectrum
hardware behaviour.

---

## Turbo Spectrum ROM and documentation

Turbo Spectrum / CoBra-2 material is based on the published Turbo
Spectrum technical specification:

https://cobrasov.org/Download/TS_Spec.pdf

The Turbo Spectrum specification identifies the associated Turbo
Spectrum software/documentation material as:

**Copyright 2022 The Cismas Foundation**

and provides it under the **MIT License**.

Applicable copyright and license notices for original Turbo Spectrum
material are retained and must remain with that material.

The Turbo Spectrum material is separate from the ESPectrum-derived
GPLv3 code described below.

---

## Modifications in this repository

This repository contains modifications and additions to the original
ESPectrum project for Turbo Spectrum support.

The modifications include, among other things:

- Turbo Spectrum custom ROM integration.
- Turbo Spectrum FE register/configuration state.
- Turbo Spectrum memory-map implementation.
- ROM disable / RAM replacement behaviour.
- RAM6 access after the ROM-to-RAM transition.
- Turbo Spectrum CPU write handling for the lower 16K address range.
- Turbo Spectrum-specific video and animation behaviour.
- Debugging and validation of the Turbo Spectrum boot sequence.

The original ESPectrum source files and copyright notices have not been
removed.
...
ESPectrum is licensed under the **GNU General Public License,
version 3 (GPLv3)**.

The original ESPectrum source files retain their copyright and license
notices. In particular, the project includes work by:

- Víctor Iborra [Eremus]
- David Crespo [dcrespo3d]

The original ESPectrum project also acknowledges the earlier projects
and contributors listed in the original ESPectrum README.

This repository contains modified and extended ESPectrum-derived code.
The ESPectrum-derived portions remain subject to the GPLv3 and the
applicable copyright notices.

The Turbo Spectrum material originating from the Turbo Spectrum
Specification is subject to its applicable copyright and MIT License
notice. The MIT License is a permissive free/open-source license and
the applicable original copyright and license notices are retained.

Nothing in this repository is intended to remove, replace or weaken
the copyright or license terms applying to third-party material.

See the `LICENSE` file in the repository for the complete GPLv3 text.

---

## Project status

Turbo Spectrum support is under active development.

The current development target is an **ESP32 Pico Kit** using the
PlatformIO `psram` environment.

The implementation is being validated against the published Turbo
Spectrum memory architecture and boot/animation behaviour.
...
`DEV_NOTES.md` contains development notes describing the Turbo Spectrum
implementation, memory mapping investigation and debugging work.

The notes are intended to document the development process and are not
a replacement for the original ESPectrum or Turbo Spectrum technical
documentation.
...
## Turbo Spectrum acknowledgements

- **Sorin Cismaș / The Cismas Foundation** for the Turbo Spectrum
  (CoBra-2) architecture, technical documentation and the published
  Turbo Spectrum material under the MIT License.
- **Nicolae Muntean** for implementing the Turbo Spectrum custom ROM
  integration and Turbo Spectrum support in ESPectrum for ESP32.
